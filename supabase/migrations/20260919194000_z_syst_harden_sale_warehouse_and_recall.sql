-- Harden atomic sales: enforce warehouse availability and exclude active recalled batches.
-- Note: stocks are legacy business/product batches without warehouse_id; warehouse_stocks remains the warehouse aggregate.
...
begin
 ...
 for v_item in select * from jsonb_array_elements(p_items) loop
   ...
   if coalesce((select quantity from warehouse_stocks ws where ws.warehouse_id=p_warehouse_id and ws.product_id=v_product),0) < v_qty then
     raise exception 'insufficient warehouse stock for product %',v_product;
   end if;
 ...
 values(p_business_id,p_party_id,v_user,coalesce(p_discount_amount,0),greatest(v_total-coalesce(p_paid_amount,0),0),
        coalesce(p_paid_amount,0)>=v_total,coalesce(p_tax_amount,0),coalesce(p_paid_amount,0),v_total,0,
        p_payment_type,p_invoice_number,now(),jsonb_build_object('posted_by_rpc',true),p_idempotency_key)
 ...
 for v_item in select * from jsonb_array_elements(p_items) loop
   ...
   values(p_business_id,v_sale_id,v_product,v_qty,
          coalesce((v_item->>'unit_price')::numeric,(select sales_price from products where id=v_product)),
          coalesce((v_item->>'tax_amount')::numeric,0),coalesce((v_item->>'discount_amount')::numeric,0),
          (coalesce((v_item->>'unit_price')::numeric,(select sales_price from products where id=v_product))*v_qty)
          +coalesce((v_item->>'tax_amount')::numeric,0)-coalesce((v_item->>'discount_amount')::numeric,0),null);
   v_remaining:=v_qty;
   for v_stock in
     select s.* from stocks s
     where s.business_id=p_business_id and s.product_id=v_product and s.product_stock>0
       and (s.expire_date is null or s.expire_date>=current_date)
       and not exists(
         select 1 from recall_events r
         where r.business_id=p_business_id and r.status in ('open','active','initiated')
           and r.product_id=v_product
           and (r.batch_lot_number is null or r.batch_lot_number=s.batch_no)
       )
     order by s.expire_date nulls last,s.id for update
   loop
     exit when v_remaining=0;
     v_take:=least(v_remaining,v_stock.product_stock);
   ...
   update warehouse_stocks set quantity=quantity-v_qty,updated_at=now()
   where business_id=p_business_id and warehouse_id=p_warehouse_id and product_id=v_product;
 ...
 values(p_business_id,v_user,'create','sale',v_sale_id,'Sale posted atomically with FEFO and warehouse stock control',
        jsonb_build_object('invoice_number',p_invoice_number,'total_amount',v_total,'warehouse_id',p_warehouse_id));
 return v_sale_id;
