...
class PrescriptionPolicyTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->prescription = Prescription::factory()->create(['business_id' => $business1->id]);
    }
    ...
    {
        $this->assertTrue($this->policy->view($this->user, $this->prescription));
    }

    public function test_view_denies_non_owner(): void
    {
        $this->assertFalse($this->policy->view($this->otherUser, $this->prescription));
    }
    ...
    {
        $this->assertTrue($this->policy->update($this->user, $this->prescription));
    }

    public function test_update_denies_non_owner(): void
    {
        $this->assertFalse($this->policy->update($this->otherUser, $this->prescription));
    }

    public function test_delete_allows_owner(): void
    {
        $this->assertTrue($this->policy->delete($this->user, $this->prescription));
    }

    public function test_delete_denies_non_owner(): void
    {
        $this->assertFalse($this->policy->delete($this->otherUser, $this->prescription));
    }
}
