                ...
                <div class="card-bodys">
                    ...
                    <div class="p-4">
                        <form action="{{ route('admin.loyalty.update', $program) }}" method="POST">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label for="name" class="form-label">{{ __('Program Name') }} <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="name" name="name" value="{{ $program->name }}" required>
                                </div>
                                <div class="col-md-6">
                                    <label for="points_per_currency" class="form-label">{{ __('Points per Currency Unit') }} <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <label for="min_points_for_redemption" class="form-label">{{ __('Min Points for Redemption') }} <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <label for="expiry_days" class="form-label">{{ __('Point Expiry (Days)') }}</label>
                                    <input type="number" class="form-control" id="expiry_days" name="expiry_days" value="{{ $program->expiry_days }}" min="0">
                                </div>
                                <div class="col-md-6">
                                    <label for="is_active" class="form-label">{{ __('Status') }}</label>
                                    <select class="form-select" id="is_active" name="is_active">
                                        <option value="1" {{ $program->is_active ? 'selected' : '' }}>{{ __('Active') }}</option>
                                        <option value="0" {{ !$program->is_active ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                                    </select>
                                </div>
                                <div class="col-md-6 d-flex align-items-end">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_default" name="is_default" value="1" {{ $program->is_default ? 'checked' : '' }}>
                                        <label class="form-check-label" for="is_default">{{ __('Set as Default Program') }}</label>
                                    </div>
                                </div>
                            </div>
                            <div class="mt-4">
