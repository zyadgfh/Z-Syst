...
@section('title', __('Online Store Analytics'))
...
<div class="container-fluid m-h-100">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="dashboard-title">{{ __('Online Store Analytics') }}</h2>
            <p class="dashboard-subtitle">{{ __('Insights from your online pharmacy store') }}</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.customer-orders.index') }}" class="btn btn-secondary">
                <i class="fas fa-list me-2"></i>{{ __('View Orders') }}
            </a>
            <select onchange="window.location.href='?period='+this.value" class="form-select" style="width: auto;">
                @foreach ([7, 14, 30, 60, 90] as $p)
                    <option value="{{ $p }}" @selected($period == $p)>{{ $p }} {{ __('days') }}</option>
                @endforeach
            </select>
        </div>
    </div>
            ...
            <div class="card kpi-card p-4">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="kpi-label">{{ __('Pending Orders') }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <div class="card kpi-card p-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="kpi-icon" style="background: rgba(236, 72, 153, 0.1);">
        ...
        <div class="col-md-6">
            <div class="card kpi-card p-4">
                <div class="d-flex align-items-center gap-3">
                    <div class="kpi-icon" style="background: rgba(20, 184, 166, 0.1);">
                        <i class="fas fa-user-plus" style="color: #14b8a6;"></i>
                    </div>
                    <div>
                        <div class="kpi-value">{{ $kpis['new_customers'] }}</div>
                        <div class="kpi-label">{{ __('New Customers') }} ({{ $period }} {{ __('days') }})</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8 mb-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">{{ __('Revenue Over Time') }}</h5>
                </div>
                <div class="card-body">
        ...
        <div class="col-lg-4 mb-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('Orders by Status') }}</h5>
                </div>
                <div class="card-body">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @if ($topProducts->isEmpty())
                        <div class="text-center py-4 text-muted">{{ __('No sales data yet.') }}</div>
                    @else
                            ...
                            @foreach ($recentOrders as $order)
                                <a href="{{ route('admin.customer-orders.show', $order) }}"
                                   class="list-group-item list-group-item-action d-flex justify-content-between align-items-center px-0">
                                    ...
                                    <div class="text-end">
                                        <div class="fw-bold" style="color: #15803d;">${{ number_format($order->total_amount, 2) }}</div>
