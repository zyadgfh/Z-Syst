...
@section('title')
    {{ __('Supplier Dashboard') }} — {{ $supplier->name }}
@endsection
                ...
                <div class="table-header p-16 d-flex align-items-center justify-content-between flex-wrap gap-3">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button class="btn btn-success btn-sm" onclick="showPaymentModal()">
                            <i class="fas fa-money-bill me-1"></i>{{ __('Record Payment') }}
                        </button>
                        @endif
                        <a href="{{ route('admin.purchases.index') }}?party_id={{ $supplier->id }}" class="btn btn-outline-primary btn-sm">
                            <i class="fas fa-receipt me-1"></i>{{ __('View Purchases') }}
                        </a>
                    </div>
                </div>
                ...
                <div class="p-16">
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-3">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body text-center">
                                    <div class="text-muted mb-1">{{ __('Current Balance') }}</div>
                                    <h3 class="fw-bold {{ $balance > 0 ? 'text-danger' : 'text-success' }}">
                                        {{ number_format($balance, 2) }}
                                    </h3>
                                    <small class="text-muted">{{ __('Amount owed to supplier') }}</small>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body text-center">
                                    <div class="text-muted mb-1">{{ __('Total Purchases') }}</div>
                                    <h3 class="fw-bold text-primary">{{ number_format($summary['total_purchases'], 2) }}</h3>
                                    <small class="text-muted">{{ $summary['transaction_count'] }} {{ __('transactions') }}</small>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body text-center">
                                    <div class="text-muted mb-1">{{ __('Total Returns') }}</div>
                        ...
                        <div class="col-md-3">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body text-center">
                                    <div class="text-muted mb-1">{{ __('Total Payments') }}</div>
                    ...
                    <div class="row g-4 mb-4">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <table class="table table-borderless mb-0">
                                        ...
                                        <tr><td class="text-muted">{{ __('Status') }}</td><td><span class="badge bg-{{ $supplier->status === 'active' ? 'success' : 'secondary' }}">{{ ucfirst($supplier->status ?? 'active') }}</span></td></tr>
                                    </table>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-header"><strong>{{ __('Recent Purchases') }}</strong></div>
                                <div class="card-body p-0">
                                    <div class="table-responsive">
                                        <table class="table table-hover table-sm mb-0">
                                            <thead><tr><th>{{ __('Invoice') }}</th><th>{{ __('Date') }}</th><th class="text-end">{{ __('Total') }}</th><th class="text-end">{{ __('Due') }}</th></tr></thead>
                                            <tbody>
                                                ...
                                                @empty
                                                <tr><td colspan="4" class="text-center text-muted py-3">{{ __('No purchases yet.') }}</td></tr>
                                                @endforelse
                                            </tbody>
                                        </table>
                                    </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <strong><i class="fas fa-history me-2"></i>{{ __('Transaction Ledger') }}</strong>
                            <span class="badge bg-secondary">{{ $ledger->count() }} {{ __('entries') }}</span>
                        </div>
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        @empty
                                        <tr><td colspan="7" class="text-center text-muted py-4">{{ __('No transactions found.') }}</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('Record Payment to Supplier') }}</h5>
...
<script>
    ...
    function submitPayment() {
        ...
        if (!formData.get('amount') || parseFloat(formData.get('amount')) <= 0) {
            toastr.error('{{ __("Please enter a valid payment amount.") }}');
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-check me-1"></i>{{ __("Record Payment") }}';
            }
        })
        .catch(() => {
            toastr.error('{{ __("Error recording payment.") }}');
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-check me-1"></i>{{ __("Record Payment") }}';
        });
    }
