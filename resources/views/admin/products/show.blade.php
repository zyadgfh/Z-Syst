...
@section('title')
    {{ $product->productName }} — {{ __('products.Item Details') }}
@endsection
...
<div class="container-fluid m-h-100">
            ...
            <div>
                ...
                <div class="d-flex gap-2 align-items-center">
                    ...
                    @if($product->prescription_required)
                        <span class="badge badge-soft-warning"><i class="fas fa-prescription me-1"></i>{{ __('products.Rx Required') }}</span>
                    @endif
                    @if($product->controlled_item)
                        <span class="badge badge-soft-danger"><i class="fas fa-exclamation-triangle me-1"></i>{{ __('products.Controlled') }}</span>
                    @endif
                </div>
            </div>
        </div>
        <div class="d-flex gap-2">
            @if($product->barcode)
                <button class="btn btn-info" onclick="openBarcodePrintModal()">
                    <i class="fas fa-barcode me-1"></i>{{ __('purchases.Print Barcode') }}
                </button>
            @endif
            @can('update', $product)
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    <div class="text-muted small mb-1">{{ __('products.Current Stock') }}</div>
                    ...
                    @can('update', $product)
                        <button class="btn btn-sm btn-outline-primary mt-2" onclick="openStockAdjustModal()">
                            <i class="fas fa-plus-minus me-1"></i>{{ __('products.Adjust Stock') }}
                        </button>
                    @endcan
                </div>
            </div>
        ...
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    <div class="text-muted small mb-1">{{ __('products.Selling Price') }}</div>
                    <h3 class="mb-0 text-primary">{{ number_format($kpis['current_selling_price'], 2) }}</h3>
                    @if($product->wholesale_price)
                        <small class="text-muted">{{ __('products.Wholesale') }}: {{ number_format($product->wholesale_price, 2) }}</small>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    <div class="text-muted small mb-1">{{ __('products.Avg Cost') }}</div>
                    <h3 class="mb-0">{{ number_format($kpis['average_cost'], 2) }}</h3>
                    <small class="text-muted">{{ __('products.Margin') }}: {{ number_format($kpis['profit_margin'], 1) }}%</small>
                </div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center">
                    <div class="text-muted small mb-1">{{ __('common.Stock Status') }}</div>
                    <h3 class="mb-0">
                        @if($kpis['reorder_status'] === 'out_of_stock')
                            <span class="text-danger">{{ __('products.Out of Stock') }}</span>
                        @elseif($kpis['reorder_status'] === 'low_stock')
                            <span class="text-warning">{{ __('products.Low Stock') }}</span>
                        @elseif($kpis['reorder_status'] === 'reorder')
                            <span class="text-info">{{ __('products.Reorder') }}</span>
                        @else
                            <span class="text-success">{{ __('products.In Stock') }}</span>
                        @endif
                    </h3>
                    <small class="text-muted">{{ __('orders.Sold') }}: {{ number_format($kpis['total_sold']) }} | {{ __('products.Purchased') }}: {{ number_format($kpis['total_purchased']) }}</small>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <ul class="nav nav-tabs" id="itemTabs" role="tablist">
                <li class="nav-item">
                    <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#overview" type="button">
                        <i class="fas fa-info-circle me-1"></i>{{ __('products.Overview') }}
                    </button>
                </li>
                <li class="nav-item">
                    <button class="nav-link" data-bs-toggle="tab" data-bs-target="#pricing" type="button">
                        <i class="fas fa-tag me-1"></i>{{ __('products.Pricing') }}
                    </button>
                </li>
                <li class="nav-item">
            ...
            <div class="tab-content pt-4">
                ...
                <div class="tab-pane fade show active" id="overview">
                    <div class="row">
                        <div class="col-md-8">
                            <h6 class="text-muted mb-3">{{ __('gateways.Basic Information') }}</h6>
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    <table class="table table-sm">
                                        <tr><td class="text-muted" class="w-40pct">{{ __('products.Product Name') }}</td><td><strong>{{ $product->productName }}</strong></td></tr>
                                        @if($product->scientific_name)
                                            <tr><td class="text-muted">{{ __('products.Scientific Name') }}</td><td>{{ $product->scientific_name }}</td></tr>
                                        @endif
                                        @if($product->commercial_name)
                                            <tr><td class="text-muted">{{ __('products.Commercial Name') }}</td><td>{{ $product->commercial_name }}</td></tr>
                                        @endif
                                        <tr><td class="text-muted">{{ __('common.SKU') }}</td><td><code>{{ $product->sku ?? $product->productCode ?? '-' }}</code></td></tr>
                                        <tr><td class="text-muted">{{ __('products.Internal Code') }}</td><td>{{ $product->internal_code ?? '-' }}</td></tr>
                                        <tr><td class="text-muted">{{ __('common.Category') }}</td><td>{{ $product->category->categoryName ?? '-' }}</td></tr>
                                        @if($product->subcategory)
                                            <tr><td class="text-muted">{{ __('products.Subcategory') }}</td><td>{{ $product->subcategory->categoryName }}</td></tr>
                                        @endif
                                        <tr><td class="text-muted">{{ __('products.Manufacturer') }}</td><td>{{ $product->manufacturer->name ?? '-' }}</td></tr>
                                        @if($product->brand)
                                            <tr><td class="text-muted">{{ __('products.Brand') }}</td><td>{{ $product->brand->name }}</td></tr>
                                        @endif
                                    </table>
                                </div>
                                <div class="col-md-6">
                                    <table class="table table-sm">
                                        @if($product->dosage_form)
                                            <tr><td class="text-muted" class="w-40pct">{{ __('products.Dosage Form') }}</td><td>{{ $product->dosage_form }}</td></tr>
                                        @endif
                                        @if($product->strength)
                                            <tr><td class="text-muted">{{ __('products.Strength') }}</td><td>{{ $product->strength }}</td></tr>
                                        @endif
                                        @if($product->active_ingredient)
                                            <tr><td class="text-muted">{{ __('products.Active Ingredient') }}</td><td>{{ $product->active_ingredient }}</td></tr>
                                        @endif
                                        @if($product->concentration)
                                            <tr><td class="text-muted">{{ __('products.Concentration') }}</td><td>{{ $product->concentration }}</td></tr>
                                        @endif
                                        @if($product->route_of_administration)
                                            <tr><td class="text-muted">{{ __('products.Route') }}</td><td>{{ $product->route_of_administration }}</td></tr>
                                        @endif
                                        <tr><td class="text-muted">{{ __('products.Unit') }}</td><td>{{ $product->unit->unitName ?? '-' }}</td></tr>
                                        @if($product->medicine_type)
                                            <tr><td class="text-muted">{{ __('common.Type') }}</td><td>{{ $product->medicine_type->name }}</td></tr>
                                        @endif
                                        <tr><td class="text-muted">{{ __('products.Refrigerated') }}</td><td>{{ $product->refrigerated ? __('common.Yes') : __('products.No') }}</td></tr>
                                    </table>
                                </div>
                            </div>

                            @if($product->description || $product->notes)
                                <h6 class="text-muted mb-2">{{ __('common.Description') }}</h6>
                                <p>{{ $product->description ?? '-' }}</p>
                                @if($product->notes)
                                    <h6 class="text-muted mb-2">{{ __('common.Notes') }}</h6>
                        ...
                        <div class="col-md-4">
                            <h6 class="text-muted mb-3">{{ __('products.Inventory Settings') }}</h6>
                            <table class="table table-sm">
                                <tr><td class="text-muted">{{ __('products.Track Inventory') }}</td><td>{{ $product->track_inventory ? __('common.Yes') : __('products.No') }}</td></tr>
                                <tr><td class="text-muted">{{ __('products.Allow Negative Stock') }}</td><td>{{ $product->allow_negative_stock ? __('common.Yes') : __('products.No') }}</td></tr>
                                <tr><td class="text-muted">{{ __('products.Alert Quantity') }}</td><td>{{ $product->alert_qty }}</td></tr>
                                <tr><td class="text-muted">{{ __('products.Reorder Point') }}</td><td>{{ $product->reorder_point }}</td></tr>
                                @if($product->reorder_quantity)
                                    <tr><td class="text-muted">{{ __('products.Reorder Qty') }}</td><td>{{ $product->reorder_quantity }}</td></tr>
                                @endif
                                @if($product->minimum_stock)
                                    <tr><td class="text-muted">{{ __('products.Minimum Stock') }}</td><td>{{ $product->minimum_stock }}</td></tr>
                                @endif
                                @if($product->maximum_stock)
                                    <tr><td class="text-muted">{{ __('products.Maximum Stock') }}</td><td>{{ $product->maximum_stock }}</td></tr>
                                @endif
                            </table>

                            <h6 class="text-muted mb-3 mt-4">{{ __('products.Expiration') }}</h6>
                            <table class="table table-sm">
                                <tr><td class="text-muted">{{ __('products.Track Expiration') }}</td><td>{{ $product->track_expiration ? __('common.Yes') : __('products.No') }}</td></tr>
                                @if($product->expiration_warning_days)
                                    <tr><td class="text-muted">{{ __('products.Warning Days') }}</td><td>{{ $product->expiration_warning_days }} {{ __('common.days') }}</td></tr>
                                @endif
                                @if($product->minimum_remaining_shelf_life)
                                    <tr><td class="text-muted">{{ __('products.Min Shelf Life') }}</td><td>{{ $product->minimum_remaining_shelf_life }} {{ __('common.days') }}</td></tr>
                                @endif
                            </table>

                            @if($product->storage_instructions)
                                <h6 class="text-muted mb-3 mt-4">{{ __('products.Storage') }}</h6>
                                <p class="text-muted">{{ $product->storage_instructions }}</p>
                            @endif

                            <h6 class="text-muted mb-3 mt-4">{{ __('products.Audit') }}</h6>
                            <table class="table table-sm">
                                <tr><td class="text-muted">{{ __('products.Created') }}</td><td>{{ $product->created_at?->format('d M Y H:i') }}</td></tr>
                                <tr><td class="text-muted">{{ __('products.Updated') }}</td><td>{{ $product->updated_at?->format('d M Y H:i') }}</td></tr>
                                @if($product->createdByUser)
                                    <tr><td class="text-muted">{{ __('products.Created By') }}</td><td>{{ $product->createdByUser->name }}</td></tr>
                                @endif
                                @if($product->updatedByUser)
                                    <tr><td class="text-muted">{{ __('products.Updated By') }}</td><td>{{ $product->updatedByUser->name }}</td></tr>
                                @endif
                            </table>
                        </div>
                    </div>
                ...
                <div class="tab-pane fade" id="pricing">
                    <div class="row">
                        <div class="col-md-6">
                            <h6 class="text-muted mb-3">{{ __('products.Price Information') }}</h6>
                            <table class="table table-bordered">
                                <tr><td class="text-muted">{{ __('products.Purchase Price (excl. tax)') }}</td><td><strong>{{ number_format($product->purchase_without_tax ?? 0, 2) }}</strong></td></tr>
                                <tr><td class="text-muted">{{ __('products.Purchase Price (incl. tax)') }}</td><td><strong>{{ number_format($product->purchase_with_tax ?? 0, 2) }}</strong></td></tr>
                                <tr><td class="text-muted">{{ __('products.Selling Price') }}</td><td><strong class="text-primary">{{ number_format($product->sales_price ?? 0, 2) }}</strong></td></tr>
                                <tr><td class="text-muted">{{ __('products.Wholesale Price') }}</td><td>{{ number_format($product->wholesale_price ?? 0, 2) }}</td></tr>
                                @if($product->minimum_selling_price)
                                    <tr><td class="text-muted">{{ __('products.Min Selling Price') }}</td><td>{{ number_format($product->minimum_selling_price, 2) }}</td></tr>
                                @endif
                                @if($product->special_price)
                                    <tr><td class="text-muted">{{ __('products.Special Price') }}</td><td class="text-success fw-bold">{{ number_format($product->special_price, 2) }}</td></tr>
                                @endif
                                <tr><td class="text-muted">{{ __('products.Profit Margin') }}</td><td><span class="badge badge-soft-success">{{ number_format($product->profit_margin, 1) }}%</span></td></tr>
                                <tr><td class="text-muted">{{ __('products.Tax Type') }}</td><td>{{ ucfirst($product->tax_type ?? 'exclusive') }}</td></tr>
                                @if($product->tax)
                                    <tr><td class="text-muted">{{ __('products.Tax Rate') }}</td><td>{{ $product->tax->rate }}% ({{ $product->tax->name }})</td></tr>
                                @endif
                            </table>
                        </div>
                        <div class="col-md-6">
                            <h6 class="text-muted mb-3">{{ __('products.Profit Analysis') }}</h6>
                            @php
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="card border-0 bg-light">
                                        <div class="card-body text-center">
                                            <div class="text-muted small">{{ __('products.Gross Profit per Unit') }}</div>
                                ...
                                <div class="col-6">
                                    <div class="card border-0 bg-light">
                                        <div class="card-body text-center">
                                            <div class="text-muted small">{{ __('products.Total Revenue (Sold)') }}</div>
                ...
                <div class="tab-pane fade" id="inventory">
                    <h6 class="text-muted mb-3">{{ __('products.Stock Batches') }}</h6>
                        ...
                        <table class="table table-bordered table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('common.Purchase Price') }}</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($stock->expire_date)
                                                @if($stock->expire_date < now())
                                                    <span class="badge badge-soft-danger">{{ $stock->expire_date->format('d/m/Y') }} ({{ __('common.Expired') }})</span>
                                        ...
                                        <td>
                                            @if($stock->productStock > 0)
                                                <span class="badge badge-soft-success">{{ __('products.In Stock') }}</span>
                                            @else
                                                <span class="badge badge-soft-secondary">{{ __('products.Empty') }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="text-center text-muted py-3">{{ __('products.No stock batches found.') }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="tab-pane fade" id="suppliers">
                    <h6 class="text-muted mb-3">{{ __('products.Supplier Relationships') }}</h6>
                            ...
                            <table class="table table-bordered table-sm">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('products.Item Code') }}</th>
                                        <th>{{ __('common.Purchase Price') }}</th>
                                        <th>{{ __('products.Lead Time') }}</th>
                                        <th>{{ __('products.Min Order') }}</th>
                                        <th>{{ __('products.Preferred') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($product->itemSuppliers as $itemSupplier)
                                        <tr>
                                            ...
                                            <td>{{ $itemSupplier->lead_time_days ? $itemSupplier->lead_time_days . ' ' . __('common.days') : '-' }}</td>
                                            <td>{{ $itemSupplier->minimum_order_quantity ?? '-' }}</td>
                                            <td>
                                                @if($itemSupplier->is_preferred)
                                                    <span class="badge badge-soft-success"><i class="fas fa-star me-1"></i>{{ __('products.Preferred') }}</span>
                                                @else
                    ...
                    @else
                        <div class="text-center py-4 text-muted">
                            ...
                            <p>{{ __('products.No supplier relationships configured.') }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="tab-pane fade" id="barcodes">
                    <h6 class="text-muted mb-3">{{ __('products.Product Barcodes') }}</h6>
                    @if($product->barcode)
                        <div class="mb-3">
                            <strong>{{ __('products.Primary Barcode') }}:</strong> <code class="fs-5">{{ $product->barcode }}</code>
                    ...
                    @endif
                    @if($product->gtin)
                        <div class="mb-3"><strong>{{ __('products.GTIN') }}:</strong> <code>{{ $product->gtin }}</code></div>
                    @endif
                    ...
                    @if($product->barcodes->count() > 0)
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                <thead>
                                    <tr>
                                        <th>{{ __('products.Barcode Number') }}</th>
                                        <th>{{ __('common.Type') }}</th>
                                        <th>{{ __('common.Status') }}</th>
                                        <th>{{ __('products.Created') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($barcode->is_active)
                                                    <span class="badge badge-soft-success">{{ __('common.Active') }}</span>
                                                @else
                                                    <span class="badge badge-soft-secondary">{{ __('common.Inactive') }}</span>
                                                @endif
                                            </td>
                    ...
                    @else
                        <div class="text-center py-4 text-muted">
                            ...
                            <p>{{ __('products.No additional barcodes registered.') }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="tab-pane fade" id="stock-history">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-2">
                            <x-stat-card value="+{{ $stockMovementStats['total_in'] }}" label="{{ __('products.Total In') }}" color="success" icon="fas fa-arrow-down" size="sm" />
                        </div>
                        <div class="col-md-2">
                            <x-stat-card value="-{{ $stockMovementStats['total_out'] }}" label="{{ __('products.Total Out') }}" color="danger" icon="fas fa-arrow-up" size="sm" />
                        </div>
                        <div class="col-md-2">
                            <x-stat-card value="{{ $stockMovementStats['net_change'] >= 0 ? '+' : '' }}{{ $stockMovementStats['net_change'] }}" label="{{ __('products.Net Change') }}" color="{{ $stockMovementStats['net_change'] >= 0 ? 'primary' : 'warning' }}" icon="fas fa-exchange-alt" size="sm" />
                        </div>
                        <div class="col-md-2">
                            <x-stat-card value="{{ $stockMovementStats['total_adjustments'] }}" label="{{ __('products.Adjustments') }}" color="info" icon="fas fa-sliders-h" size="sm" />
                        </div>
                        <div class="col-md-2">
                            <x-stat-card value="{{ $stockMovementStats['total_movements'] }}" label="{{ __('products.Total Events') }}" color="secondary" icon="fas fa-list" size="sm" />
                        </div>
                    </div>
                    ...
                    <form method="GET" class="row g-2 mb-3" id="stockHistoryFilter">
                        ...
                        <div class="col-md-2">
                            <select name="movement_type" class="form-select form-select-sm">
                                ...
                                <option value="in" {{ request('movement_type') === 'in' ? 'selected' : '' }}>{{ __('products.Stock In') }}</option>
                                <option value="out" {{ request('movement_type') === 'out' ? 'selected' : '' }}>{{ __('products.Stock Out') }}</option>
                                <option value="adjustment" {{ request('movement_type') === 'adjustment' ? 'selected' : '' }}>{{ __('products.Adjustment') }}</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <input type="date" name="movement_from" class="form-control form-control-sm" value="{{ request('movement_from') }}" placeholder="{{ __('common.From') }}">
                        </div>
                        <div class="col-md-2">
                            <input type="date" name="movement_to" class="form-control form-control-sm" value="{{ request('movement_to') }}" placeholder="{{ __('common.To') }}">
                        </div>
                        <div class="col-md-2">
                            <select name="movement_user_id" class="form-select form-select-sm">
                                <option value="">{{ __('products.All Users') }}</option>
                        ...
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-sm btn-outline-primary w-100"><i class="fas fa-filter me-1"></i>{{ __('common.Filter') }}</button>
                        </div>
                        <div class="col-md-2">
                            <a href="{{ route('admin.items.show', $product->id) }}?tab=stock-history" class="btn btn-sm btn-outline-secondary w-100"><i class="fas fa-times me-1"></i>{{ __('products.Clear') }}</a>
                        </div>
                    </form>
                    ...
                    <x-data-table
                        :headers="[
                            ['label' => __('products.Date & Time'), 'style' => 'white-space:nowrap'],
                            __('common.Type'),
                            __('common.Qty'),
                            __('products.Before'),
                            __('products.After'),
                        ...
                        empty-text="{{ __('products.No stock movements recorded for this item.') }}"
                    >
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($movement->movement_type === 'in')
                                        <span class="badge badge-soft-success"><i class="fas fa-arrow-down me-1"></i>{{ __('products.In') }}</span>
                                    @elseif($movement->movement_type === 'out')
                                        <span class="badge badge-soft-danger"><i class="fas fa-arrow-up me-1"></i>{{ __('products.Out') }}</span>
                                    @else
                                        <span class="badge badge-soft-info"><i class="fas fa-exchange-alt me-1"></i>{{ __('products.Adjust') }}</span>
                                    @endif
                                </td>
                ...
                <div class="tab-pane fade" id="print-history">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="text-muted mb-0">{{ __('products.Barcode Label Print History') }}</h6>
                        <span class="badge badge-soft-secondary">{{ $printHistory->count() }} {{ __('products.prints') }}</span>
                    </div>
                                ...
                                <tbody>
                                    @foreach($printHistory as $print)
                                        <tr>
                                            ...
                                            <td><strong>{{ $print->quantity }}</strong> {{ __('products.label(s)') }}</td>
                    ...
                    @else
                        <div class="text-center py-5 text-muted">
                            ...
                            <h5>{{ __('products.No barcode labels printed yet') }}</h5>
                            <p>{{ __('products.Click "Print Barcode" to generate and download barcode labels.') }}</p>
                        </div>
                    @endif
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6">
                            <h6 class="text-muted mb-3">{{ __('products.Recent Sales') }}</h6>
                            <div class="table-responsive">
                                <table class="table table-sm table-hover">
                                    <thead>
                                        <tr><th>{{ __('common.Invoice') }}</th><th>{{ __('common.Date') }}</th><th>{{ __('common.Amount') }}</th></tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        @empty
                                            <tr><td colspan="3" class="text-center text-muted">{{ __('products.No sales recorded.') }}</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div class="col-md-6">
                            <h6 class="text-muted mb-3">{{ __('products.Recent Purchases') }}</h6>
                            <div class="table-responsive">
                                <table class="table table-sm table-hover">
                                    <thead>
                                        <tr><th>{{ __('common.Invoice') }}</th><th>{{ __('common.Date') }}</th><th>{{ __('common.Amount') }}</th></tr>
                                    ...
                                    <tbody>
                                        ...
                                        @empty
                                            <tr><td colspan="3" class="text-center text-muted">{{ __('products.No purchases recorded.') }}</td></tr>
                                        @endforelse
                                    </tbody>
                                </table>
                            </div>
                ...
                <div class="tab-pane fade" id="timeline">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="text-muted mb-0"><i class="fas fa-stream me-1"></i>{{ __('products.Activity Timeline') }}</h6>
                        <span class="badge badge-soft-secondary">{{ count($timelineEvents) }} {{ __('products.events') }}</span>
                    </div>
                    ...
                    @else
                        <div class="text-center py-5 text-muted">
                            ...
                            <h6>{{ __('products.No activity recorded yet') }}</h6>
                            <small>{{ __('products.Stock movements, price changes, and other events will appear here.') }}</small>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-barcode me-2"></i>{{ __('products.Print Barcode Labels') }}
                </h5>
            ...
            <div class="modal-body">
                <div class="alert alert-info d-flex align-items-center mb-3">
                    ...
                    <span>
                        {{ __('products.Barcode') }}: <strong><code>{{ $product->barcode ?? __('products.Not set') }}</code></strong>
                        @if($product->barcode_type)
                ...
                <form id="barcodePrintForm" onsubmit="return false;">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('products.Number of Labels') }} <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" name="quantity" id="barcodeQty" value="4" min="1" max="200" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('products.Label Size') }}</label>
                            <select class="form-select" name="size" id="barcodeSize">
                                <option value="small">{{ __('products.Small') }} (50×28mm)</option>
                                <option value="standard" selected>{{ __('products.Standard') }} (70×35mm)</option>
                                <option value="large">{{ __('products.Large') }} (90×45mm)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    @if($product->allStocks->count() > 1)
                        <div class="mb-3">
                            <label class="form-label">{{ __('products.Batch (optional)') }}</label>
                            <select class="form-select" name="batch_id">
                                <option value="">{{ __('products.First batch') }}</option>
                                @foreach($product->allStocks as $stock)
                                    <option value="{{ $stock->id }}">
                                        {{ $stock->batch_no ?? __('purchases.Batch') }} #{{ $stock->id }}
                                        — {{ $stock->productStock }} {{ __('products.units') }}
                                        @if($stock->expire_date)
                                            | {{ __('products.Exp') }}: {{ $stock->expire_date->format('d/m/Y') }}
                                        @endif
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <h6 class="text-muted mb-2">{{ __('products.Label Options') }}</h6>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="show_scientific" id="bpShowScientific" value="1" checked>
                                <label class="form-check-label" for="bpShowScientific">{{ __('products.Show scientific name') }}</label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="show_price" id="bpShowPrice" value="1">
                                <label class="form-check-label" for="bpShowPrice">{{ __('products.Show selling price') }}</label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="show_expiry" id="bpShowExpiry" value="1" checked>
                                <label class="form-check-label" for="bpShowExpiry">{{ __('products.Show expiry date') }}</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="show_batch" id="bpShowBatch" value="1" checked>
                                <label class="form-check-label" for="bpShowBatch">{{ __('products.Show batch number') }}</label>
                            </div>
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="show_code" id="bpShowCode" value="1" checked>
                                <label class="form-check-label" for="bpShowCode">{{ __('products.Show SKU / code') }}</label>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="stockAdjustModalLabel">
                    <i class="fas fa-plus-minus me-2"></i>{{ __('products.Adjust Stock') }} — {{ $product->productName }}
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-info d-flex align-items-center mb-3">
                    <i class="fas fa-info-circle me-2"></i>
                    <span>{{ __('products.Current total stock:') }} <strong>{{ $kpis['current_stock'] }}</strong> {{ __('products.units') }}</span>
                </div>
                ...
                <form id="stockAdjustForm">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('products.Adjustment Type') }} <span class="text-danger">*</span></label>
                            <select class="form-select" id="adjustType" required onchange="onAdjustTypeChange()">
                                <option value="in">{{ __('products.Stock In (Add)') }}</option>
                                <option value="out">{{ __('products.Stock Out (Remove)') }}</option>
                                <option value="adjustment">{{ __('products.Set Exact Quantity') }}</option>
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label" id="adjustQtyLabel">{{ __('products.Quantity to Add') }} <span class="text-danger">*</span></label>
                            <input type="number" class="form-control" id="adjustQty" min="1" value="1" required>
                            <small class="text-muted" id="adjustQtyHelp">{{ __('products.Enter the quantity to add to stock.') }}</small>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="adjustBatchNo" placeholder="{{ __('products.Optional batch/lot number') }}">
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('common.Purchase Price') }}</label>
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label">{{ __('products.Reference / Notes') }}</label>
                            <input type="text" class="form-control" id="adjustNotes" placeholder="{{ __('products.e.g. New delivery, Return from customer...') }}">
                        </div>
                    </div>
                    ...
                    <div id="batchSelectorSection" class="d-none">
                        <label class="form-label"><strong>{{ __('products.Select Batch to Remove From') }}</strong></label>
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered">
                                <thead>
                                    <tr>
                    ...
                    <div class="card bg-light" id="adjustPreview" class="d-none">
                        <div class="card-body py-2">
                            <div class="d-flex justify-content-between">
                                <span>{{ __('products.Current Stock:') }}</span>
                                <strong>{{ $kpis['current_stock'] }}</strong>
                            </div>
                            <div class="d-flex justify-content-between">
                                <span>{{ __('products.Adjustment:') }}</span>
                                <strong id="previewAdjustment" class="text-success">+1</strong>
                            </div>
                            <hr class="my-1">
                            <div class="d-flex justify-content-between">
                                <span><strong>{{ __('products.New Stock:') }}</strong></span>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="submitStockAdjust()" id="adjustSubmitBtn">
                    <i class="fas fa-save me-1"></i>{{ __('products.Apply Adjustment') }}
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    function openBarcodePrintModal() {
        if (!'{{ $product->barcode }}') {
            toastr.warning('{{ __('products.No barcode set for this item. Edit the item to add one.') }}');
            return;
        }
    ...
    document.getElementById('barcodeSize')?.addEventListener('change', function() {
        const labels = { small: '{{ __('products.Small') }}', standard: '{{ __('products.Standard') }}', large: '{{ __('products.Large') }}' };
    ...
    function downloadBarcodePdf() {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>{{ __('products.Generating...') }}';
        ...
        setTimeout(() => {
            hiddenForm.remove();
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-download me-1"></i>{{ __('products.Download PDF') }}';
    ...
    function onAdjustTypeChange() {
        ...
        switch(type) {
            case 'in':
                qtyLabel.innerHTML = '{{ __('products.Quantity to Add') }} <span class="text-danger">*</span>';
                qtyHelp.textContent = '{{ __('products.Enter the quantity to add to stock.') }}';
                qtyInput.min = 1;
                break;
            case 'out':
                qtyLabel.innerHTML = '{{ __('products.Quantity to Remove') }} <span class="text-danger">*</span>';
                qtyHelp.textContent = '{{ __('products.Enter the quantity to remove from stock.') }}';
                qtyInput.min = 1;
                qtyInput.max = currentStock;
                batchSection.classList.remove('d-none');
                break;
            case 'adjustment':
                qtyLabel.innerHTML = '{{ __('products.New Total Quantity') }} <span class="text-danger">*</span>';
                qtyHelp.textContent = '{{ __('products.Set the exact total quantity for this item.') }}';
                qtyInput.min = 0;
                qtyInput.max = '';
                break;
        }
    ...
    function submitStockAdjust() {
        ...
        // Validate
        if (!qty || qty < 0) {
            toastr.error('{{ __('products.Please enter a valid quantity.') }}');
            return;
        }
        ...
        if (type === 'out') {
            ...
            if (!selectedBatch) {
                toastr.error('{{ __('products.Please select a batch to remove from.') }}');
                return;
            }
        }
        ...
        .catch(err => {
            toastr.error('{{ __('products.An error occurred while adjusting stock.') }}');
            console.error(err);
        })
        .finally(() => {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-save me-1"></i>{{ __('products.Apply Adjustment') }}';
        });
    }
        ...
        if (ctx) {
            new Chart(ctx.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: '{{ __('products.Selling Price') }}',
                        ...
                        {
                            label: '{{ __('common.Purchase Price') }}',
                        ...
                        {
                            label: '{{ __('products.Wholesale Price') }}',
    ...
    function submitStockAdjust() {
        ...
        .then(data => {
            if (data.success) {
                ...
                toastr.error(data.message || '{{ __('products.Error adjusting stock') }}');
            }
        })
        .catch(() => {
            toastr.error('{{ __('products.Network error') }}');
        })
        .finally(() => {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-save me-1"></i>{{ __('products.Apply Adjustment') }}';
    ...
    if (typeof Echo !== 'undefined') {
        Echo.private(`item.${productId}.stock`)
            .listen('.stock.updated', (e) => {
                ...
                const msg = type === 'in'
                    ? `{{ __('products.Stock added') }}: +${qty} ({{ __('common.Total') }}: ${e.new_total_stock})`
                    : type === 'out'
                    ? `{{ __('products.Stock removed') }}: -${qty} ({{ __('common.Total') }}: ${e.new_total_stock})`
                    : `{{ __('products.Stock adjusted') }}: ${e.new_total_stock}`;

                toastr.info(msg, `{{ __('products.Stock Update') }} by ${e.movement.user}`);
