@forelse($categories as $category)
    <tr class="table-content">
        ...
        <td class="table-single-content">{{ $categories->perPage() * ($categories->currentPage() - 1) + $loop->iteration }}</td>

        <td class="text-start table-single-content">{{ $category->name }}</td>
        <td class="text-start table-single-content">{{ Str::limit($category->description, 35, '...') }}</td>
        ...
        <td class="table-single-content d-print-none">
            <div class="dropdown table-action">
                ...
                <ul class="dropdown-menu">
                    @can('business-categories-update')
                        <li><a  href="{{ route('admin.business-categories.edit', $category->id) }}">
    ...
    <tr>
        <td colspan="7" class="text-center py-5">
            <div class="d-flex flex-column align-items-center">
                <i class="fas fa-th-large fa-3x mb-3" class="text-gray-border"></i>
                <h5 class="text-heading-dark fw-600">{{ __('business.No categories yet') }}</h5>
                <p class="fs-14 text-subtle">{{ __('business.Create your first category to organize products.') }}</p>
                @can('business-categories-create')
                    <a href="{{ route('admin.business-categories.create') }}" class="btn btn-primary mt-2">
                        <i class="fas fa-plus me-1"></i>{{ __('dashboard.Add Category') }}
                    </a>
                @endcan
            </div>
        </td>
