@foreach($plans as $plan)
    <tr class="table-content">
        ...
        @endcan
        <td class="table-single-content">{{ ($plans->perPage() * ($plans->currentPage() - 1)) + $loop->iteration }}</td>
        <td  class="text-start table-single-content">{{ $plan->subscriptionName }} </td>
        <td>{{ $plan->duration }} </td>
        <td class="fw-bold text-dark table-single-content">{{ $plan->offerPrice ? currency_format($plan->offerPrice) : '' }} </td>
        <td class="fw-bold text-dark table-single-content">{{ currency_format($plan->subscriptionPrice) }} </td>
