        ...
        <div class="card">
            <div class="card-bodys ">
                <div class="table-header p-16">
                    <h4>{{__('business.Plan List')}}</h4>
                    @can('plans-create')
                        <a type="button" href="{{route('admin.plans.create')}}" class="add-order-btn rounded-2 {{ Route::is('admin.plans.create') ? 'active' : '' }}" class="btn btn-primary" ><i class="fas fa-plus-circle me-1"></i>{{ __('business.Create Plans') }}</a>
                    @endcan
                </div>


                <div class="table-header justify-content-center border-0 text-center d-none d-block d-print-block">
                    <h4 class="mt-2">{{ __('business.Plan List') }}</h4>
                </div>
                    ...
                    <form action="{{ route('admin.plans.filter') }}" method="post" class="filter-form" table="#plans-data">
                        ...
                        <div class="table-top-left d-flex gap-3 ">
                            ...
                            <div class="table-search position-relative">
                                <input class="form-control searchInput" type="text" name="search" placeholder="{{ __('common.Search...') }}" value="{{ request('search') }}">
            ...
            <div class="delete-item delete-show d-none multi-delete-container">
                <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                    ...
                    <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('admin.plans.delete-all') }}">{{ __('common.Delete') }}</button>
                </div>
            </div>
            ...
            <div class="responsive-table table-container">
                <table class="table" id="datatable">
                    ...
                    <tr>
                        ...
                        <th class="text-start table-header-content">{{ __('business.Subscription Name') }}</th>
                        <th class="table-header-content">{{ __('business.Duration') }}</th>
                        <th class="table-header-content">{{ __('business.Offer Price') }}</th>
                        <th class="table-header-content">{{ __('business.Subscription Price') }}</th>
