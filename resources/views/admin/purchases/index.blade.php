...
@section('title')
    {{ __('purchases.Purchase Invoices') }}
@endsection
        ...
        <div class="card">
            <div class="card-bodys">
                <div class="table-header p-16 d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <h4 class="mb-0">{{ __('purchases.Purchase Invoices') }}</h4>
                    <div class="d-flex gap-2">
                        @can('purchases-create')
                        <a href="{{ route('admin.purchases.create') }}" class="btn btn-primary">
                            <i class="fas fa-plus me-1"></i>{{ __('dashboard.New Purchase') }}
                        </a>
                        @endcan
                    </div>
                </div>
                ...
                <div class="p-16 border-top">
                    <form method="GET" action="{{ route('admin.purchases.index') }}" class="row g-3 align-items-end">
                        <div class="col-md-3">
                            ...
                            <input type="text" name="search" class="form-control" placeholder="{{ __('purchases.Invoice #, Supplier, Note...') }}" value="{{ $filters['search'] ?? '' }}">
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="status" class="form-select">
                                ...
                                <option value="received" @selected(($filters['status'] ?? '') == 'received')>{{ __('purchases.Received') }}</option>
                                <option value="canceled" @selected(($filters['status'] ?? '') == 'canceled')>{{ __('purchases.Cancelled') }}</option>
                                <option value="returned_partially" @selected(($filters['status'] ?? '') == 'returned_partially')>{{ __('purchases.Returned (Partial)') }}</option>
                                <option value="returned_fully" @selected(($filters['status'] ?? '') == 'returned_fully')>{{ __('purchases.Returned (Full)') }}</option>
                            </select>
                        </div>
                        <div class="col-md-1">
                ...
                <div class="table-responsive p-16">
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                <th>{{ __('purchases.Invoice #') }}</th>
                                <th>{{ __('purchases.Supplier') }}</th>
                                <th>{{ __('common.Branch') }}</th>
                                <th>{{ __('common.Date') }}</th>
                                <th>{{ __('common.User') }}</th>
                                <th class="text-end">{{ __('common.Total') }}</th>
                                <th class="text-end">{{ __('purchases.Paid') }}</th>
                                <th class="text-end">{{ __('purchases.Remaining') }}</th>
                        ...
                        <tbody>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item" href="{{ route('admin.purchases.show', $purchase->id) }}"><i class="fas fa-eye me-2"></i>{{ __('common.View') }}</a></li>
                                            @if($purchase->status === 'received' && auth()->user()->can('purchases-edit'))
                                            <li><a class="dropdown-item" href="{{ route('admin.purchases.edit', $purchase->id) }}"><i class="fas fa-edit me-2"></i>{{ __('common.Edit') }}</a></li>
                                            @endif
                                            @if(in_array($purchase->status, ['received', 'returned_partially']) && auth()->user()->can('purchases-create'))
                                            <li><a class="dropdown-item" href="{{ route('admin.purchases.returns.create') }}?purchase_id={{ $purchase->id }}"><i class="fas fa-undo me-2"></i>{{ __('purchases.Return') }}</a></li>
                                            @endif
                                            @if($purchase->status === 'received' && auth()->user()->can('purchases-delete'))
                                            <li><hr class="dropdown-divider"></li>
                                            <li><a class="dropdown-item text-danger" href="#" onclick="cancelPurchase({{ $purchase->id }}, '{{ $purchase->invoiceNumber }}')"><i class="fas fa-ban me-2"></i>{{ __('common.Cancel') }}</a></li>
                                            @endif
                                        </ul>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colspan="10" class="text-center py-4 text-muted">
                                    ...
                                    <p>{{ __('purchases.No purchase invoices found.') }}</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
...
<script>
    function cancelPurchase(id, invoiceNumber) {
        if (!confirm(`{{ __('purchases.Are you sure you want to cancel purchase') }} ${invoiceNumber}?`)) return;
        ...
        .catch(() => toastr.error('{{ __('purchases.Error cancelling purchase.') }}'));
    }
</script>
@endpush
