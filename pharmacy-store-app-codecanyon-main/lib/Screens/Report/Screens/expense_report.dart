...
class _ExpenseReportState extends State<ExpenseReport> {
  ...
  final PagingController<int, ExpenseReportData> pageController = PagingController(firstPageKey: 1);
  ...
  Future<void> fetchPurchaseDataList(int pageKey) async {
    try {
      final list = await incomes.getExpenseReportList(
      ...
      if (list != null) {
        ...
        totalIncome = list.totalExpense ?? 0;
    ...
    return AcnooScafoldWidget(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: PagedListView<int, ExpenseReportData>(
              shrinkWrap: true,
              padding: EdgeInsets.zero,
              pagingController: pageController,
              scrollController: _scrollController,
              physics: const AlwaysScrollableScrollPhysics(),
              builderDelegate: PagedChildBuilderDelegate<ExpenseReportData>(
  ...
  //---Table data
  Widget _buildDataRow(ExpenseReportData item, ThemeData theme) {
    return Table(
      border: TableBorder.symmetric(
        inside: BorderSide.none,
        outside: BorderSide.none,
      ...
      children: [
        TableRow(
          children: [
            _buildTableCell(
              '${item.expanseFor ?? 'n/a'}\n${item.expenseDate != null ? DateFormat('d MMM yyyy').format(DateTime.parse(item.expenseDate.toString())) : 'n/a'}',
