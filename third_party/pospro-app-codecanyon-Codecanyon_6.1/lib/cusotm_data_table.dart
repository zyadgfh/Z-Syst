...
class DataRow {
  ...
  ///
  /// If non-null, tapping the row will call this callback. If
  /// null (including [onDoubleTap], [onLongPress], [onTapCancel] and [onTapDown]),
  /// tapping the row will attempt to select the row (if
  /// [DataRow.onSelectChanged] is provided).
  final GestureTapCallback? onTap;

  /// Called if the row is long-pressed.
  ///
  /// If non-null, tapping the row will invoke this callback. If
  /// null (including [onTap] [onDoubleTap], [onTapCancel] and [onTapDown]),
  /// tapping the row will attempt to select the row (if
  /// [DataRow.onSelectChanged] is provided).
  ...
  bool get _debugInteractive => onSelectChanged != null || onTap != null || onLongPress != null || cells.any((DataCell cell) => cell._debugInteractive);
}
...
class DataTable extends StatelessWidget {
  ...
  bool get _debugInteractive {
    return columns.any((DataColumn column) => column._debugInteractive) || rows.any((DataRow row) => row._debugInteractive);
  }
  ...
  Widget _buildHeadingCell({
    ...
    label = Semantics(
      ...
      child: Row(
        ...
        children: <Widget>[
          if (headingRowAlignment == MainAxisAlignment.center && onSort != null) const SizedBox(width: _SortArrowState._arrowIconSize + _sortArrowPadding),
          label,
    ...
    final TextStyle effectiveHeadingTextStyle = headingTextStyle ?? dataTableTheme.headingTextStyle ?? themeData.dataTableTheme.headingTextStyle ?? themeData.textTheme.titleSmall!;
  ...
  Widget _buildDataCell({
    ...
    label = Container(
      ...
      child: DefaultTextStyle(
        style: DefaultTextStyle.of(context).style.merge(effectiveDataTextStyle).copyWith(color: placeholder ? effectiveDataTextStyle.color!.withOpacity(0.6) : null),
    ...
    if (onTap != null || onDoubleTap != null || onLongPress != null || onTapDown != null || onTapCancel != null) {
      ...
      label = TableRowInkWell(
        onTap: onSelectChanged != null ? () => onSelectChanged!() : null,
  ...
  Widget build(BuildContext context) {
    ...
    final Iterable<DataRow> rowsWithCheckbox = displayCheckboxColumn ? rows.where((DataRow row) => row.onSelectChanged != null) : <DataRow>[];
    ...
    if (displayCheckboxColumn) {
      ...
      for (final DataRow row in rows) {
        ...
        tableRows[rowIndex].children[0] = _buildCheckbox(
          ...
          onRowTap: row.onTap ?? (row.onSelectChanged == null ? null : () => row.onSelectChanged?.call(!row.selected)),
