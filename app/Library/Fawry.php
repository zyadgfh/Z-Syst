...
class Fawry
{
    ...
    public static function make_payment($array)
    {
        ...
        $data['merchant_code'] = $array['merchant_code'] ?? '';
        $data['merchant_key'] = $array['merchant_key'] ?? '';
        $data['merchant_name'] = $array['merchant_name'] ?? '';

        Session::put('fawry_credentials', $data);

        // Fawry is manual - redirect to payment page with instructions
        return request()->expectsJson() ? route('fawry.view') : redirect()->route('fawry.view');
    }

    public function view()
    {
        if (Session::has('fawry_credentials')) {
            $info = Session::get('fawry_credentials');
            $gateway = Gateway::where('status', 1)->findOrFail($info['gateway_id']);

            return view('payments.fawry', compact('info', 'gateway'));
        }
        abort(404);
    }
    ...
    {
        if (Session::has('fawry_credentials')) {
            $order_info = Session::get('fawry_credentials');

            // For Fawry, this is manual verification
            $data['payment_method'] = 'Fawry';
            ...
            Session::forget('fawry_credentials');

            return request()->expectsJson() ?
            Fawry::redirect_if_payment_success() :
            redirect(Fawry::redirect_if_payment_success());
        }
        abort(404);
    }
}