...
class OrangeCashGateway extends BasePaymentGateway
{
    public function getGatewayType(): string
    {
        return CompanyPaymentGateway::GATEWAY_ORANGE_CASH;
    }
    ...
    public function processPayment(array $paymentData): PaymentTransaction
    {
        ...
        try {
            ...
            if (empty($customerPhone)) {
                throw new \Exception('Customer phone number is required for Orange Cash');
            }

            // Orange Cash API integration
            $response = $this->callOrangeCashApi([
    ...
    {
        try {
            $response = $this->callOrangeCashStatusApi([
                'merchant_id' => $this->config['merchant_id'],
    ...
    public function processRefund(PaymentTransaction $transaction, ?float $amount = null): array
    {
        try {
            ...
            $response = $this->callOrangeCashRefundApi([
                'merchant_id' => $this->config['merchant_id'],
    ...
    private function callOrangeCashApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.orangecash.com.eg'
            : 'https://sandbox.orangecash.com.eg';
    ...
    private function callOrangeCashStatusApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.orangecash.com.eg'
            : 'https://sandbox.orangecash.com.eg';
    ...
    private function callOrangeCashRefundApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.orangecash.com.eg'
            : 'https://sandbox.orangecash.com.eg';
