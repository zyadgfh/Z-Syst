...
class InstaPayGateway extends BasePaymentGateway
{
    public function getGatewayType(): string
    {
        return CompanyPaymentGateway::GATEWAY_INSTAPAY;
    }
    ...
    public function processPayment(array $paymentData): PaymentTransaction
    {
        ...
        try {
            ...
            if (empty($customerPhone)) {
                throw new \Exception('Customer phone number is required for InstaPay');
            }

            // InstaPay API integration
            $response = $this->callInstaPayApi([
    ...
    {
        try {
            $response = $this->callInstaPayStatusApi([
                'merchant_id' => $this->config['merchant_id'],
    ...
    public function processRefund(PaymentTransaction $transaction, ?float $amount = null): array
    {
        try {
            ...
            $response = $this->callInstaPayRefundApi([
                'merchant_id' => $this->config['merchant_id'],
    ...
    private function callInstaPayApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.instapay.com.eg'
            : 'https://sandbox.instapay.com.eg';
    ...
    private function callInstaPayStatusApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.instapay.com.eg'
            : 'https://sandbox.instapay.com.eg';
    ...
    private function callInstaPayRefundApi(array $data): array
    {
        $baseUrl = $this->config['environment'] === 'production'
            ? 'https://api.instapay.com.eg'
            : 'https://sandbox.instapay.com.eg';
